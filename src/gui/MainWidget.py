...
from PySide6.QtWidgets import QGroupBox, QHBoxLayout, QTabWidget, QWidget, QLabel, QPushButton, QVBoxLayout
...
from gui.qtutils import newGroup
...
class MainWidget(QWidget, LogHolder):
    def __init__(self, config: Config, close_callback: Callable) -> None:
        ...
        self.first_tab.layout().addWidget(newGroup("Mouse Control", EnabledMouseActionsControl(self.config)))

        self.second_tab = QWidget()
        self.tab_widget.addTab(self.second_tab, "Config")
        self.second_tab.setLayout(QVBoxLayout())
        self.second_tab.layout().addWidget(newGroup("Monitor Setup", MonitorConfigControl(self.config)))
